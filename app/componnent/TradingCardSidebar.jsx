...
const TradingCardSidebar = ({ cards, addCard, Done, removeCard, editmood, seteidtmood, doneloading }) => {
    return (
        <div className="w-full border-b lg:border-r border-gray-200 bg-white h-full shawow-md px-2 md:px-7 lg:px-8 py-2 z-20">
            ...
            <div className="w-full lg:w-full flex flex-row lg:flex-col gap-4 items-center h-[70px] lg:h-fit snap-mandatory overflow-x-scroll cursor-grab lg:overflow-y-scroll scrollbar-hide lg:max-h-[77vh] lg:min-h-[77vh] -translate-y-4 lg:translate-y-0">
                {cards?.map((card, idx) => (
                    <div
                        key={idx}
                        className={`w-[45px] lg:w-full relative rounded-xl p-1 z-0 snap-start`}
                    >
                        <TradingCardThumnail
                            card={card}
                            Done={Done}
